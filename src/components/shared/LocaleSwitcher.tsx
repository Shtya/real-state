...
export default function LocaleSwitcher({ Trigger }: LocaleSwitcherProps) {
    ...
    return (
        <>
            {Trigger ? (
            ...
            ) : (<button onClick={toggleLocale} className="cursor-pointer flex items-center gap-1 sm:gap-2 transition rounded-full text-base sm:text-lg lg:text-xl" disabled={isPending}>
                ...
                <span className="hidden md:block font-bold"> {nextLocale.toUpperCase()}</span>
            </button>)}
        </>
    );
}